...
mod tests {
    ...
    #[tokio::test]
    async fn test_lpush_handler_existing_list() {
        ...
        let result = lpush_handler(server.clone(), key.clone(), args).await;
    ...
    #[tokio::test]
    async fn test_lpush_handler_new_list() {
        let server = setup().await;
        let key = "key".to_string();
        let args = vec![Value::BulkString("new_item".to_string())];
        let result = lpush_handler(server.clone(), key.clone(), args).await;
    ...
    #[tokio::test]
    async fn test_lpush_handler_invalid_value_type() {
        let server = setup().await;
        let key = "key".to_string();
    ...
    #[tokio::test]
    async fn test_lpush_handler_non_list_value() {
        let server = setup().await;
        let key = "key".to_string();
        let redis_item = RedisItem {
